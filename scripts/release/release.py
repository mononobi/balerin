...
class ReleaseManager:
    ...
    BALERIN_VERSION_FILE = '../../src/balerin/__init__.py'
    DEFAULT_CHANGE = 'patch'
    DEFAULT_IS_BETA = False
    PIPFILE = '../../Pipfile'
    PYTHON_VERSION = 'python{version}'
    PYPI_PACKAGE_NAME = 'balerin'
    MAX_MINOR = 50
    MAX_PATCH = 50
    ...
    def _get_current_version(self):
        """
        gets current balerin version from balerin package.

        :returns: tuple(int major, int minor, int patch, str beta)
        :rtype: tuple
        """

        with io.open(self.BALERIN_VERSION_FILE, 'rt', encoding='utf8') as version_file:
    ...
    def _set_version(self, old_version, new_version):
        ...
        with open(self.BALERIN_VERSION_FILE, 'r') as file:
            file_data = file.read()

        file_data = file_data.replace(formatted_old, formatted_new)

        with open(self.BALERIN_VERSION_FILE, 'w') as file:
